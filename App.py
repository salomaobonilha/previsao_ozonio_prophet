...
def load_model():
    with open('model_O3_prophet.json','r') as file_in:
        modelo = model_from_json(json.load(file_in))
        return modelo

modelo = load_model()

st.title('Previsão de níveis de ozônio (O3) utilizando a biblioteca Phophet')
...
if st.session_state.previsao_feita:
    ...
    st.write("Tabela contendo as previsões de concentração de ozônio (ug/m³) para os próximos {} dias:".format(dias))
